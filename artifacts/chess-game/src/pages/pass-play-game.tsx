  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        ...
        <ChessBoard game={game} onMove={handleMove} disabled={!!resultStatus && !showEndAd} />

        <div className="w-full max-w-lg mt-6 flex justify-between px-2">
           <div className="flex items-center gap-3 bg-zinc-900 px-4 py-2 rounded-lg border border-zinc-800 transition-opacity" style={{ opacity: game.turn() === 'w' ? 1 : 0.5 }}>
      ...
      <AdModal open={showEndAd} onComplete={handleEndAdComplete} title="Advertisement" />
      {/* Show result modal only after ad completes */}
      {adCompleted && <ResultModal open={true} status={resultStatus || 'draw'} isPassAndPlay={true} />}
    </div>
  );
}
